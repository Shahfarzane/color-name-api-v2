...
const MAX_COLORS_PER_REQUEST = parseInt(process.env.MAX_COLORS_PER_REQUEST || '170');
 ...
 */
function parseHexColors(values: string): { valid: string[]; invalid: string[] } {
 ...
 */
function errorResponse(c: Context, status: number, message: string, extra?: Record<string, unknown>) {
  return c.json(
    {
      error: {
        status,
...
colors.get('/', async (c) => {
  ...
  if (invalidColors.length > 0) {
    return errorResponse(c, 400, `Invalid hex colors: ${invalidColors.join(', ')}`);
  }
  ...
  if (hexColors.length > MAX_COLORS_PER_REQUEST) {
    return errorResponse(c, 400, `Maximum ${MAX_COLORS_PER_REQUEST} colors per request`);
  }

  // Get color names
  const colorResults = findColors.getNamesForValues(hexColors, noduplicates, list);
