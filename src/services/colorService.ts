...
import type { RawColor, ColorListMeta } from '../types';
...
export async function initColorService(): Promise<void> {
  ...
  console.log(`[ColorService] Total colors: ${Object.values(colorLists).reduce((sum, list) => sum + list.length, 0)}`);
...
export function getFindColors(): FindColors {
  if (!findColors) {
    throw new Error('ColorService not initialized. Call initColorService() first.');
  }
  return findColors;
}
...
export function getServiceStats(): {
  ...
  return {
    ...
    totalColors: Object.values(colorLists).reduce((sum, list) => sum + list.length, 0),
